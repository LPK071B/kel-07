...
if selected == "Materi":
    ...
    st.write("N = bobot zat terlarut (mg) / (FP x V titran x BE zat terlarut (mL)")
    ...
    st.write("% Kadar = (V titran (mL) x N titran (mgrek/mL) x BE titrat (mg/mgrek) x 10^-3 x FP x 100%) / V titrat (mL)")
    
    st.write(":blue[RUMUS KADAR TANPA FP]")
    st.write("% Kadar = (V titran (mL) x N titran (mgrek/mL) x BE titrat (mg/mgrek) x 10^-3 x 100%) / V titrat (mL)")
    
             
    st.write(":red[EXTRA MATERI ASIDIMETRI DAN PENETAPAN KADAR NAOH DAN NA2CO3 DALAM CAMPURAN SECARA WARDER]")
    ...
    st.write("Sumber :")
    st.write("www.wardayacollege.com")
    st.write("alasains.wordpress.com")
    st.write("www.pakarkimia.com")
             
if selected == 'About Us':
    st.write("dibuat oleh: kelompok 7")
